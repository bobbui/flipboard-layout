...
(function (window, undefined) {
    ...
    $.Flips.prototype = {
        ...
        _adjustLayout: function (page) {
            ...
            this.$flipPages.each(function (i) {
                ...
                if (i === page - 1) {
                    ...
                    //IE hack, hide the front page
                    if (isIE) {
                        $page.find('div.front').css('display', 'none');
                    }
                }
                else if (i < page) {
                    $page.css({
        ...
        _loadEvents: function () {
            ...
            this.$flipPages.find('.box').on('touchend click.flips', function (event) {
                ...
                var $box = $(this),
                    ...
                    $overlay = $('<div class="overlay">close</div>').css({
                        ...
                        '-ms-transition': 'opacity ' + transitionProp.speed + 'ms ' + transitionProp.timingfunction
                    }).prependTo($('body')),
                    prop = {
                    ...
                    $placeholder = $box.clone().css({
                        ...
                        '-ms-transition': 'all ' + transitionProp.speed + 'ms ' + transitionProp.timingfunction
                    })
                        .insertAfter($overlay)
                        .end()
		    ...
		    .hide()
		    .end()
                    .on('webkitTransitionEnd.flips oTransitionEnd.flips msTransitionEnd.flips transitionend.flips ', function (event) {
        ...
        _turnPage: function (angle, update) {
            //IE hack, show the front when angle < 90
            //          hide the front when angle > 90
            if (isIE) {
                if (angle >= 90)
                    this.$flippingPage.find('div.front').css('display', 'none');
                else
                    this.$flippingPage.find('div.front').css('display', 'block');
            }
            ...
            if (!update) {
                this.$flippingPage.css({
                    ...
                    'transition': 'transform ' + this.flipSpeed + 'ms ' + this.options.fliptimingfunction,
                    '-ms-transition': '-ms-transform ' + this.flipSpeed + 'ms ' + this.options.fliptimingfunction
                });
            }
            ...
            if (update) {
                ...
                if (this.$afterOverlay) {
                    ...
                    this.$afterOverlay.css({
                        ...
                        'transition': 'opacity ' + afterspeed + 'ms ' + this.options.fliptimingfunction + ' ' + afterdelay + 'ms',
                        '-ms-transition': 'opacity ' + afterspeed + 'ms ' + this.options.fliptimingfunction + ' ' + afterdelay + 'ms',
                ...
                if (this.$beforeOverlay) {
                    ...
                    this.$beforeOverlay.css({
                        ...
                        'transition': 'opacity ' + beforespeed + 'ms ' + this.options.fliptimingfunction + ' ' + beforedelay + 'ms',
                        '-ms-transition': 'opacity ' + beforespeed + 'ms ' + this.options.fliptimingfunction + ' ' + beforedelay + 'ms',
